...
function GetOpenTestDataSetAndUploadToFileShare() {
  ...
  echo "  Downloading Open Test Data (TNO) v0.27.0 from OSDU"
  wget --quiet -O $OPEN_TEST_DATA_ARCHIVE_FILE_NAME https://community.opengroup.org/osdu/platform/data-flow/data-loading/open-test-data/-/archive/v0.27.0/open-test-data-v0.27.0.tar.gz
  ...
  tar -xzf $OPEN_TEST_DATA_ARCHIVE_FILE_NAME -C $TEMP_DATA_DIR/datasets/documents --strip-components=5 open-test-data-v0.27.0/rc--1.0.0/1-data/3-provided/USGS_docs

  echo "  Extracting Dataset Markers"
  tar -xzf $OPEN_TEST_DATA_ARCHIVE_FILE_NAME -C $TEMP_DATA_DIR/datasets/markers --strip-components=5 open-test-data-v0.27.0/rc--1.0.0/1-data/3-provided/markers

  echo "  Extracting Dataset Trajectories"
  tar -xzf $OPEN_TEST_DATA_ARCHIVE_FILE_NAME -C $TEMP_DATA_DIR/datasets/trajectories --strip-components=5 open-test-data-v0.27.0/rc--1.0.0/1-data/3-provided/trajectories

  echo "  Extracting Dataset Well Logs"
  tar -xzf $OPEN_TEST_DATA_ARCHIVE_FILE_NAME -C $TEMP_DATA_DIR/datasets/well-logs --strip-components=5 open-test-data-v0.27.0/rc--1.0.0/1-data/3-provided/well-logs

  echo "  Extracting Schemas"
  tar -xzf $OPEN_TEST_DATA_ARCHIVE_FILE_NAME -C $TEMP_DATA_DIR/schema --strip-components=3 open-test-data-v0.27.0/rc--3.0.0/3-schema

  echo "  Extracting Templates"
  tar -xzf $OPEN_TEST_DATA_ARCHIVE_FILE_NAME -C $TEMP_DATA_DIR/templates --strip-components=3 open-test-data-v0.27.0/rc--3.0.0/5-templates

  echo "  Extracting TNO Contrib"
  tar -xzf $OPEN_TEST_DATA_ARCHIVE_FILE_NAME -C $TEMP_DATA_DIR/TNO/contrib --strip-components=5 open-test-data-v0.27.0/rc--3.0.0/1-data/3-provided/TNO

  echo "  Extracting TNO Provided"
  tar -xzf $OPEN_TEST_DATA_ARCHIVE_FILE_NAME -C $TEMP_DATA_DIR/TNO/provided --strip-components=3 open-test-data-v0.27.0/rc--3.0.0/4-instances/TNO
